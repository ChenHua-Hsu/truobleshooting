...
class Preprocessor:
    ...
    def inverse_transform_hit_r(self, e_, mask, new_padding_value):
      new_e = (((np.exp(e_))/(1.+(np.exp(e_)))) - 1e-6) / (1.-2e-6) * 45.

      new_e[mask] = (np.ones((len(new_e), 1))*new_padding_value)[mask]
      new_e = np.reshape(new_e, (-1,)) 
      return new_e

    def transform_hit_theta(self, hit_pos):
      new_pos = (hit_pos + np.pi) / (2*np.pi)
      return new_pos
    
    def inverse_transform_hit_theta(self, hit_pos, mask, new_padding_value):
      new_pos = 2*np.pi*hit_pos - np.pi
      new_pos[mask] = (np.ones((len(new_pos),1)) * new_padding_value)[mask]
      new_pos = np.reshape(new_pos, (-1,))
      return new_pos
    ...
    def transform(self, E,X,Y,Z, incident_energy):
      ...
      new_X = self.transform_hit_r(X)
      new_Y = self.transform_hit_theta(Y)
    ...
    def inverse_transform_hit(self, E,X,Y,Z, incident_energy, padding_value, new_padding_value):
      ...
      new_X = self.inverse_transform_hit_r(X, mask, new_padding_value)
      new_Y = self.inverse_transform_hit_theta(Y, mask, new_padding_value)
...
class Convertor:
    def __init__(self, dataset_name, label, padding_value = 0, device='cpu', preprocessor='datasets/test/dataset_2_padded_nentry1129To1269_preprocessor.pkl'):
        ...
        self.dataset = Evaluate_rtheta.evaluate_dataset(dataset[0], dataset[1], label_tensor, device)
        self.device  = device
