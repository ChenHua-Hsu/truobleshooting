...
class Gen(nn.Module):
    def __init__(self, n_feat_dim, embed_dim, hidden_dim, num_encoder_blocks, num_attn_heads, dropout_gen, marginal_prob_std, **kwargs):
        ...
        # Separate embedding for (time/incident energy) conditional inputs (small NN with fixed weights)
        ...
        # Simple DNN to change dimension from embed+2 to embed
    ...
    def forward(self, x, t, e, mask=None):
        ...
        e_expanded = e.unsqueeze(1).unsqueeze(2).expand(-1, x.size(1), 1)
        t_expanded = t.unsqueeze(1).unsqueeze(2).expand(-1, x.size(1), 1) 
        #embed_e_expanded = embed_e_.unsqueeze(1).expand(-1, x.size(1), -1)
        # Feed input embeddings into encoder block
        for layer in self.encoder:
            # Concatenate the dense embeddings for t and e
            x = torch.cat((x, e_expanded), dim=-1)
            x = torch.cat((x, t_expanded), dim=-1)

            # Reduce dimension from embed+2 to embed
            x = self.reduce_dim(x)
            # Each encoder block takes previous block's output as input
            x = layer(x, x_cls, mask) # Block layers
        
        # Rescale model's output (helps capture the normalisation of the true scores)
...
class Gen1(nn.Module):
    def __init__(self, n_feat_dim, embed_dim, hidden_dim, num_encoder_blocks, num_attn_heads, dropout_gen, marginal_prob_std, **kwargs):
        ...
        # Embedding: size of input (n_feat_dim) features -> size of output (embed_dim)
        self.embed = nn.Linear(n_feat_dim, embed_dim)
        self.embed1 = nn.Linear(embed_dim+2, embed_dim)
        # Separate embedding for (time/incident energy) conditional inputs (small NN with fixed weights)
        self.embed_e = nn.Sequential(GaussianFourierProjection(embed_dim=embed_dim), nn.Linear(embed_dim, embed_dim))
        self.embed_t = nn.Sequential(GaussianFourierProjection(embed_dim=embed_dim), nn.Linear(embed_dim, embed_dim))
        # Boils embedding down to single value
