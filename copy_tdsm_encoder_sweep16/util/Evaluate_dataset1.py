...
class evaluate_dataset(Dataset):
  ...
  def __init__(self, data, inE, label, mask_total,device = 'cpu'):
  ...
  def digitize(self, particle='photon',xml_bin='binning_dataset_1_photons.xml',pad_value=0.0):
...
class evaluator:
  ...
  def __init__(self, base_dataset_name, gen_dataset_name, mask_total,padding_value = 0, device='cpu', digitize=True):
    ...
    if digitize:
        gen_data     = digitize_input(gen_data,'photon','binning_dataset_1_photons.xml')
    ...
    self.dataset = evaluate_dataset(base_data, base_inE, base_label, mask_total,device)
    self.dataset.concat(evaluate_dataset(gen_data,  gen_inE,  gen_label,  mask_total,device))
