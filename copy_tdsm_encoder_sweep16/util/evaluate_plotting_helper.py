...
def plot_cell_dist_dep(shower_arr, ref_shower_arr, arg):
    """ plots voxel energies across all layers """
    plt.figure(figsize=(6, 6))
    # if arg.x_scale == 'log':
    bins = np.logspace(np.log10(5000),
                           np.log10(ref_shower_arr.max()),
                           50)
    #bins = np.linspace(5000, ref_shower_arr.max(), 100)
    # else:
    #     bins = 50

    counts_ref, _, _ = plt.hist(ref_shower_arr, bins=bins,
                                label='reference', density=True, histtype='stepfilled',
                                alpha=0.2, linewidth=2.)
    counts_data, _, _ = plt.hist(shower_arr, label='generated', bins=bins,
                                 histtype='step', linewidth=3., alpha=1., density=True)
    plt.title(r"Dep energy distribution")
    plt.xlabel(r'$E$ [MeV]')
    plt.yscale('log')
    ...
    if arg.mode in ['all', 'hist-p', 'hist']:
        filename = os.path.join(arg.output_dir,
                                'dep_energy_dataset_{}.png'.format(arg.dataset))
        plt.savefig(filename, dpi=300)
    if arg.mode in ['all', 'hist-chi', 'hist']:
        seps = _separation_power(counts_ref, counts_data, bins)
        #print("Separation power of voxel distribution histogram: {}".format(seps))
