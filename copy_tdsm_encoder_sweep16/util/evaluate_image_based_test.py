...
def prepare_low_data_for_classifier(hdf5_file, hlf_class, label, normed=True):
...
if __name__ == '__main__':
    ...
    reference_shower, reference_energy = extract_shower_and_energy(reference_file,
                                                                   which='reference')
    print("reference shower is here ------------------")
    
    
    shower = np.array(shower)
    reference_shower = np.array(reference_shower)
    print(reference_shower)
    print(len(reference_shower))
    average_shower = np.mean(shower, axis=0)
    # non_zero_mask = (reference_shower >= 0.1)
    # reference_shower = reference_shower[non_zero_mask]
    # print("reference shower is here ------------------")
    # print(reference_shower)

    # Example 2D array (shower) with shape [512, 9]
    # Replace this with your actual data
    #shower = np.random.rand(512, 9) * 0.2  # Random values between 0 and 0.2
    ...
    # # Loop through each column (cell)
    # for i in range(reference_shower.shape[1]):
    #     # Extract the column
    #     column = reference_shower[:, i]
        
    #     # Filter out values less than the threshold
    #     valid_values = column[column >= threshold]
        
    #     # Calculate the mean of the remaining values
    #     if len(valid_values) > 0:
    #         mean_val = np.mean(valid_values)
    #     else:
    #         mean_val = 0  # Or set to np.nan or some other value if no values are above the threshold
    ...
    # print("Mean values excluding numbers less than 0.1:", mean_values)

    average_shower_ref = np.mean(reference_shower, axis=0)
    fig, (ax1, ax2) = plt.subplots(2, 1, gridspec_kw={'height_ratios': [3, 1]}, figsize=(8, 6))

    # Plot: Mean energy per cell
    ax1.plot(average_shower, label='Gen', color='blue', marker='o')
    ax1.plot(average_shower_ref, label='Reference', color='red', marker = 'x')
    ax1.set_xlabel('layers')
    ax1.set_ylabel('Max voxel/dep energy')
    ax1.set_title('Max voxel/dep energy')
    ax1.legend()
    percent_diff = 100 * (average_shower_ref - average_shower) / average_shower
    ...
    # Save the plot
    plt.savefig('max voxel rate.png')
    print(shower.shape)
    print(shower.flatten().shape)
    depshower = np.sum(shower, axis=1)
    depshower_ref = np.sum(reference_shower, axis=1)
    #plot_cell_dist_dep(depshower, depshower_ref, args)
