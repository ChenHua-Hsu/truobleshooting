...
if __name__ == '__main__':
  ...
  parser.add_argument('--config_file', type=str, default = '/eos/user/c/chenhua/copy_tdsm_encoder_sweep16/configs/test_for_sde_parameters.yml', help = 'configuration file for wandb')
  parser.add_argument('--python_cfg', type=str, default = 'test_latex.py', help = 'python file to run the code')
  parser.add_argument('--n_run', type=int, default = 1, help = 'number of runs')
  ...
  #sweep_yml['parameters']['preprocessor'] = {'value': os.path.join(CWD, "datasets_new/dataset_2_padded_transform_incident_later_nentry1033To1161_preprocessor.pkl")}
  ...
  condor.write('output = %s/job_latex_$(Process).out\n'%farm_dir)
  condor.write('error  = %s/job_latex_$(Process).err\n'%farm_dir)
  condor.write('log    = %s/job_latex_$(Process).log\n'%farm_dir)
